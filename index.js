      ...
      if (localhostDB && nameuserDB && passwordDB && name101DB) {
        try {
          ...
          const [whitelistRows] = await con.execute(
            `SELECT whitelist FROM accounts WHERE id = ?`,
            [idJogo]
          );
          const hasWhitelist =
          ...
          const [idExistsRows] = await con.execute(
            `SELECT COUNT(*) AS count, discord FROM accounts WHERE id = ?`,
            [idJogo]
          );
          ...
          if (
            ...
            console.log("user dc", interaction.user.id);
            interaction.reply({
          ...
          const sql = `UPDATE accounts SET whitelist = 1, discord = ? WHERE id = ?`;
